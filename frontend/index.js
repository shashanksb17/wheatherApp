...
var weatherMap = {
	...
	20: { type: "haze", class: "", intensity: 0.5, icon: "wi-fog", name: "Foggy" }, //foggy
	21: { type: "haze", class: "", intensity: 0.5, icon: "wi-fog", name: "Haze" }, //haze
	22: {
		type: "smoke",
		class: "",
		intensity: 0.5,
...
function onResize() {
	...
	TweenMax.set(sunburst.node, {
		...
		x: sizes.card.width / 2 + sizes.card.offset.left,
...
function makeRain() {
	...
	var lineLength =
		...
		currentWeather.type == "severe" ||
		currentWeather.type == "hail"
			? 35
			: 14;
...
function makeSplash(x, type) {
	...
	var splashLength =
		type == "thunder" || type == "severe" || type == "hail" ? 30 : 20;

	// splashBounce is the max height the line will curve up
	// before falling
	var splashBounce =
		type == "thunder" || type == "severe" || type == "hail" ? 120 : 100;
	...
	// the same for both
	var speed =
		type == "thunder" || type == "severe" || type == "hail" ? 0.7 : 0.5;
	...
	var splash = outerSplashHolder.path(points.join(" ")).attr({
		fill: "none",
		stroke:
			type == "thunder" || type == "severe" || type == "hail" ? "#777" : "#86a3f9",
		strokeWidth: 1
	});
...
function makeHail() {
	...
	if (size > 4) {
		...
		newHail = outerHailHolder.circle(0, 0, size).attr({
			fill:
				currentWeather.type == "sleet" || currentWeather.type.indexOf("mix") > -1
					? "#86a3f9"
					: "#FFF"
		});
		...
		newHail = innerHailHolder.circle(0, 0, size).attr({
			fill:
				currentWeather.type == "sleet" || currentWeather.type.indexOf("mix") > -1
					? "#86a3f9"
					: "#FFF"
	...
	TweenMax.fromTo(
		...
		{ x: x - windOffset, y: y },
		{
			...
			x: x,
			ease: Power2.easeIn,
			onComplete: onHailEnd,
			onCompleteParams: [newHail, size, x, currentWeather.type]
		}
	);
...
function tick() {
	...
	for (var i = 0; i < fog.length; i++) {
		if (currentWeather.type == "haze" || currentWeather.type == "smoke") {
			...
			if (fog[i].offset > sizes.card.width)
				fog[i].offset = 0 + (fog[i].offset - sizes.card.width);
			fog[i].group.transform(
				"t" +
					fog[i].offset +
					"," +
					(sizes.card.height - settings.cloudHeight - settings.cloudSpace * i)
			);
		} else {
			if (fog[i].offset > -(sizes.card.width * 1.5))
				fog[i].offset += settings.windSpeed / (i + 1);
			if (fog[i].offset > sizes.card.width * 2.5)
				fog[i].offset = -(sizes.card.width * 1.5);
			fog[i].group.transform(
				"t" +
					fog[i].offset +
					"," +
...
function changeWeather(weather) {
	...
	switch (weather.type) {
		...
		case "rain":
			TweenMax.to(settings, 3, {
				rainCount: 20 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		case "hail":
			TweenMax.to(settings, 3, {
				rainCount: 5 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		case "severe":
		case "thunder":
			TweenMax.to(settings, 3, {
				rainCount: 30 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		default:
	...
	switch (weather.type) {
		...
		case "mix-rain-sleet":
			TweenMax.to(settings, 3, {
				hailCount: 10 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		case "sleet":
			TweenMax.to(settings, 3, {
				hailCount: 20 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		case "severe":
	...
	switch (weather.type) {
		...
		case "snow":
			TweenMax.to(settings, 3, {
				snowCount: 40 * weather.intensity,
				ease: Power2.easeInOut
			});
			break;
		default:
	...
	switch (weather.type) {
		case "sun":
			...
			TweenMax.to(sunburst.node, 4, {
				scale: 1,
				opacity: 0.8,
				x: sizes.card.offset.left + sizes.card.width / 2,
				y: sizes.card.height / 2 + sizes.card.offset.top,
				ease: Power2.easeInOut
			});
			break;
		case "cloud":
			var ypos =
				sizes.card.height / 2 - (sizes.card.height / 2) * weather.intensity;
			TweenMax.to(sun.node, 4, {
				x: sizes.card.width / 2,
				y: ypos,
				ease: Power2.easeInOut
		...
		default:
			...
			TweenMax.to(sunburst.node, 2, {
				...
				opacity: 0,
				x: sizes.card.offset.left + sizes.card.width / 2,
				y: sizes.container.height / 2 - 100,
				ease: Power2.easeInOut
			});
			break;
	}
